...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (hebrew_name !== undefined)
            updateData.hebrew_name = hebrew_name || null;
        if (death_date !== undefined) {
            updateData.death_date = death_date;
            updateData.hebrew_death_date = hebrew_death_date;
        }
        if (relationship !== undefined)
            updateData.relationship = relationship || null;
        if (notes !== undefined)
            updateData.notes = notes || null;
        if (notify_days_before !== undefined)
            updateData.notify_days_before = notify_days_before;
        await db
